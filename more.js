document.addEventListener('DOMContentLoaded', () => {
    ...
    function initNavigation() {
        ...
        links.forEach(l => { l.classList.remove('font-extrabold', 'text-gray-900'); l.classList.add('font-normal', 'text-gray-500'); });
        let target = activeLink;
        if (!target) {
            const path = window.location.pathname.split('/').pop() || 'more.html';
            links.forEach(link => { if (link.getAttribute('href') === path) target = link; });
        }
        if (target) {
